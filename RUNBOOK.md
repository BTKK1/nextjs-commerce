...
Every main-branch push runs one product-grounded Arabic live-agent monitor against OpenRouter model `z-ai/glm-5.3-flash`, the official model previously exposed through the retired OX Alpha test alias. Model fallback is hard-disabled, so this path cannot spend calls on Gemini, Qwen, DeepSeek, or any other model. The live monitor is a deployment gate: a retired, unavailable, unauthorized, or ungrounded model route cannot be promoted to production. The larger bilingual quality matrix remains isolated behind the **Run the full live GLM 5.3 Flash agent quality suite** workflow input. Neither path changes the model selected for live merchant agents.
