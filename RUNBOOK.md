...
Every main-branch push runs one product-grounded Arabic live-agent monitor against OpenRouter model `stealth/ox-alpha`. Model fallback is hard-disabled, so this path cannot spend calls on Gemini, Qwen, DeepSeek, or any other model. Because OX Alpha uses a rate-limited shared upstream pool, monitor failures are recorded in the GitHub job summary without blocking an otherwise healthy commerce deployment. The larger bilingual quality matrix remains isolated behind the **Run the real OX Alpha agent quality suite** workflow input and remains a hard failure gate when requested. Neither path changes the model selected for live merchant agents.
