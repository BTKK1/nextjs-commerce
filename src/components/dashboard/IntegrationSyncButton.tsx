...
export function IntegrationSyncButton({ provider }: { provider: string }) {
  ...
  async function sync() {
    ...
    try {
      ...
      setResult({ kind: "success", message: `${payload.recordsProcessed ?? 0} products updated` });
      router.refresh();
    } catch (error) {
    ...
    <DashboardTranslated>
      <div className="space-y-2">
        <button type="button" onClick={sync} disabled={pending} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-[12px] border border-[#CFC6F6] bg-white px-3 py-2 text-xs font-bold text-[#4A21D6] disabled:cursor-wait disabled:opacity-70">
          ...
          {t(pending ? "Syncing all products…" : "Sync products now")}
        </button>
