...
test("storefront smoke validates all products, compatibility alias, and local links", async ({ page }) => {
  ...
  await page.locator(`a[href="/product/${productSlugs[0]}"]`).first().click();
  await expect(page).toHaveURL(new RegExp(`/product/${productSlugs[0]}`));
  ...
  await expect(page.getByTestId("agent-widget")).toHaveCount(0);
  await expect(page.locator(`a[href="/ar/product/${productSlugs[0]}"]`).first()).toBeVisible();
  await page.locator(`a[href="/ar/product/${productSlugs[0]}"]`).first().click();
  await expect(page).toHaveURL(new RegExp(`/ar/product/${productSlugs[0]}`));
